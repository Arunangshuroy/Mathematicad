...
module snap_tongue(length, depth, thickness) {
    // Tongue is a prism elongated along Y, extruded along X.
    // Profile in XZ plane:
    //   points: (0,0), (depth,0), (depth, thickness*0.6), (0, thickness)
    //   The 0.6 factor on the angled tip creates the 45° lead-in chamfer.
...
module box_body() {
    difference() {
        ...
        // ── Snap groove ──────────────────────────────────────────────────────
        // A peripheral groove cut into the inner face of the rim at groove_z.
        // The groove is a rectangular channel:  depth = snap_depth + clearance,
        // height = snap_thickness + 2×clearance (bilateral clearance in Z).
        // The groove extends all the way around the perimeter (full loop) so
        // the lid engages on all four faces simultaneously — distributes load
        // evenly and prevents the lid from tilting during removal.
        translate([wall_t - snap_depth - snap_clearance,
                   wall_t - snap_depth - snap_clearance,
                   groove_z])
            difference() {
                // Groove outer boundary (defines the channel depth)
                rounded_box(
                ...
                // Groove inner boundary (the material that remains between groove and cavity)
                translate([snap_depth + snap_clearance,
                           snap_depth + snap_clearance, -0.1])
                    rounded_box(inner_w, inner_d, snap_thickness + 2*snap_clearance + 0.2,
...
module snap_lid() {
    ...
    // Front tongue (Y- face of lid interior)
    translate([lid_wall + corner_r, lid_wall, tongue_z])
        rotate([90, 0, 0])
    ...
    // Right tongue (X+ face)
    translate([lid_wall + lid_inner_w, lid_wall + corner_r, tongue_z])
        rotate([90, 0, 90])
        snap_tongue(lid_inner_d - 2*corner_r, snap_depth, snap_thickness);
}
